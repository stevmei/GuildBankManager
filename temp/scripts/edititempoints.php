...
while ($row = @mysql_fetch_assoc($result)) {
	if (toSaferValue(@$_POST["edititempoints_id"]) == $row["itemid"]) {
		$found = true;
		break;
	}
}
...
if ($found) {
	mysql_query("UPDATE ".$databasename.".".$tableprefix."itempoints SET points = ".toSaferValue(@$_POST["edititempoints_points"])." WHERE itemid = ".toSaferValue(@$_POST["edititempoints_id"]));
